...
def extract_facial_distances(image):
	...
	all_dist_dict = {"dist_l": dist_l , "dist_r": dist_r, "dist_eyes": dist_eyes, "dist_mouth_nose": dist_mouth_nose, "hght_mouth": hght_mouth, "wdth_mouth": wdth_mouth}
	norm_dict = {}
	for dist_name, dist_val in all_dist_dict.items():
		norm_dict[dist_name] = (dist_val - float(MEAN_STD[dist_name]["mean"])) / float(MEAN_STD[dist_name]["std"] )
	...
	return image, pd.DataFrame([norm_dict])
...
if __name__ == "__main__":
	...
	data = pd.read_csv("normalized_emotions.csv")
	...
	# Use stored SVM
	print("Creating SVM...")
	if os.path.isfile("svm_normalized.pickle"):
		f = open("svm_normalized.pickle", "rb")
		clf = pickle.load(f)
		f.close()
	# Create a SVM if it does not already exist
		...
		f = open("svm_normalized.pickle", "wb")
		pickle.dump(clf, f)
		f.close()
